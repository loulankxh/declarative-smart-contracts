contract Ppg {
  struct TotalWithdrawnCreatorTuple {
    uint n;
    bool _valid;
  }
  struct PRICETuple {
  ...
  mapping(address=>TotalWithdrawnCreatorTuple) totalWithdrawnCreator;
  PRICETuple PRICE;
  OwnerTuple owner;
  ...
  mapping(address=>TotalWithdrawnDevTuple) totalWithdrawnDev;
  ...
  function updateInitialMintOnInsertConstructor_r12() private    {
      initialMint = InitialMintTuple(0,true);
  }
  ...
  function totalMint(address o) private view  returns (uint) {
      if(o==owner.p) {
        uint m = totalMinted.n;
        uint i = initialMint.n;
        uint s = i+m;
        return s;
      }
  }
