contract Tether {
  ...
  function transferWithoutFee(address from,address to,uint amount) private view  returns (bool) {
      if(s==transferWithFee.from && r==transferWithFee.to) {
        uint f = transferWithFee.fee;
        uint n = transferWithFee.amount;
        return true;
      }
      if(o==transferFromWithoutFee.from && r==transferFromWithoutFee.to && n==transferFromWithoutFee.amount) {
        return true;
      }
      if(o==owner.p) {
        if(s==transferWithFee.from && f==transferWithFee.fee) {
